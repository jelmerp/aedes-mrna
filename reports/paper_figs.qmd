...
```{r}
# Create a dataframe for Laura with DEGs from all studies combined, in wide format
DE_join <- DE_res |>
  filter(tissue %in% c("LRT 3%", "LRT 12%"),
         padj < 0.05) |> 
  mutate(study = ifelse(tissue == "LRT 3%", "own3pct", "own12pct")) |> 
  select(gene_id, lfc, padj, study, hpm) |>
  bind_rows(others) |>
  filter(!study %in% c("alonso", "pascini")) |>
  group_by(gene_id, hpm) |> 
  add_count(name = "n_significant") |> 
  pivot_wider(id_cols = c(gene_id, hpm, n_significant),
              names_from = study,
              values_from = c(lfc, padj)) |>
  arrange(gene_id)

DE_join_file <- file.path(outdir, "DEGs_allstudies.tsv")
write_tsv(DE_join, DE_join_file)
```


<br>

## qPCR correlation plot
...
ggplot(qpcr) +
  ...
  labs(x = "Log-fold change (RNAseq)",
       y = "Log-fold change (qPCR)") +
...
I'm keeping the BP - CC - MF order for now.
Between having an alphabetical order & the most interesting ontology (BP) being on top
on the one hand, versus ordering by the number of significant terms on the other hand,
I think I prefer the former.
But if you still prefer to order MF - BP - CC, I can do that. 
...
I'm including this figure for consistency, but this single significant pathway
for 24 hpm should just be mentioned in the text instead.

```{r}
kegg_main |>
  filter(DE_direction == "both", time == "24 hpm") |>
  cdotplot(facet_var1 = "tissue", fill_var = "padj_log",
           facet_scales = "fixed", add_cat_id = TRUE) +
  scale_y_continuous(limits = c(0, NA),
                     expand = expansion(mult = c(0.005, 0.075)))
```
...
## Venn Diagrams comparing tissues
...
p1 <- ggvenn(DE_list6,
             ...
             stroke_color = "grey40", set_name_size = 5)
p2 <- ggvenn(DE_list24,
             show_percentage = FALSE,
             fill_color = tissue_cols, fill_alpha = 0.7,
             stroke_color = "grey40", set_name_size = 5)
...
## Venn Diagrams comparing studies

In all Venn Diagrams, panel A is 6 hpm and panel B is 24 hpm.

I went for separate figures with 3% and 12%,
since otherwise a Venn diagram with 5 groups is needed and that gets very crowded.
If we do need a figure with all 5 groups, we could consider an "upset plot".
...
list6 <- list(DE6_LRT3$gene_id, alfpar_6h$gene_id, camargo_6h$gene_id)
names(list6) <- c("LRT 3%", "Alfonso-Parra et al.", "Camargo et al.")

list24 <- list(amaro$gene_id, DE24_LRT3$gene_id, alfpar_24h$gene_id, camargo_24h$gene_id)

names(list24) <- c("MAG-injected vs.\nnon-injected\n(Amaro et al.)",
                   "LRT 3%",
                   "Mated vs. virgin (LRT)\n(Alfonso-Parra et al.)",
                   "Mated vs. virgin\n(Spermathecae)\n(Camargo et al.)")
...
p1 <- ggvenn(list6, show_percentage = TRUE,
             fill_color = study_cols[1:3], fill_alpha = 0.4,
             stroke_color = "grey40", set_name_size = 4, text_size = 3) +
  coord_cartesian(clip = "off")
p2 <- ggvenn(list24, show_percentage = TRUE,
             fill_color = study_cols2, fill_alpha = 0.8,
             stroke_color = "grey40", set_name_size = 3.5, text_size = 3) +
...
```{r, fig.width=9, fig.height=4}
# Create lists for Venn diagrams
list6 <- list(DE6_LRT12$gene_id, alfpar_6h$gene_id, camargo_6h$gene_id)
names(list6) <- c("LRT 12%", "Alfonso-Parra et al.", "Camargo et al.")

list24 <- list(amaro$gene_id, DE24_LRT12$gene_id, 
               alfpar_24h$gene_id, camargo_24h$gene_id)
names(list24) <- c("MAG-injected vs.\nnon-injected\n(Amaro et al.)",
                   "LRT 12%",
                   "Mated vs. virgin (LRT)\n(Alfonso-Parra et al.)",
                   "Mated vs. virgin\n(Spermathecae)\n(Camargo et al.)")
...
p2 <- ggvenn(list24, show_percentage = TRUE,
             fill_color = study_cols2, fill_alpha = 0.8,
             stroke_color = "grey40", set_name_size = 3.5, text_size = 3) +
  theme(plot.margin = margin(0.1, 1.5, 0.1, 0.1, "cm")) +
  coord_cartesian(clip = "off")
