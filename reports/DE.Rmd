...
top10 <- function(DE_df, ftissue, fcontrast,
  ...
  DE_df <- DE_df %>%
    filter(tissue %in% ftissue,
           contrast %in% fcontrast,
           padj < 0.05) %>%
    arrange(padj) %>%
...
### Across all tissues & contrasts

<br>

```{r}
top10(DE_res, tissues, contrasts, tissue_col = TRUE)
```
...
```{r}
top10(DE_res, tissues, contrasts[1], contrast_col = FALSE, tissue_col = TRUE)
```
...
```{r}
top10(DE_res, tissues, contrasts[2], contrast_col = FALSE, tissue_col = TRUE)
```
...
```{r}
top10(DE_res, tissues, contrasts[3], contrast_col = FALSE, tissue_col = TRUE)
```
...
```{r}
top10(DE_res, tissues, contrasts[4], contrast_col = FALSE, tissue_col = TRUE)
```
...
```{r}
top10(DE_res, tissues[1], contrasts)
```
...
```{r}
top10(DE_res, tissues[2], contrasts)
```
...
```{r}
top10(DE_res, tissues[3], contrasts)
```
...
```{r}
top10(DE_res, tissues[4], contrasts)
```
