...
sig_by_comp %>% 
  rename("group A" = trt_a, "group B" = trt_b,
         ...
         LRT3 = lrt, LRT12 = lrt12) %>%
  make_dt(simple_mode = TRUE)
```

### LVP06 vs AKH06 in LRT3

This is the only gene that is DE between a LVP vs AKH comparison:
...
```{r, out.width="90%"}
volc_plot(DE_res, ftissue = "lrt12")
```
...
#### `r contrasts[3]` {.tabset .tabset-fade .tabset-pills}

```{r}
t10 <- top10(DE_res, tissues, contrasts[3],
             contrast_col = FALSE, tissue_col = TRUE)
```
...
#### `r contrasts[4]` {.tabset .tabset-fade .tabset-pills}

```{r}
t10 <- top10(DE_res, tissues, contrasts[4],
             contrast_col = FALSE, tissue_col = TRUE)
```
...
#### `r tissues[4]` {.tabset .tabset-fade .tabset-pills}

```{r}
t10 <- top10(DE_res, tissues[4], contrasts)
```
...
```{r}
t10 <- top10(DE_res, tissue, contrasts[3])
```
...
```{r}
t10 <- top10(DE_res, tissue, contrasts[4])
```
...
### LRT 12% {.tabset .tabset-fade .tabset-pills}
...
```{r}
msig <- multisig(DE_res, tissues = "lrt12")
```
...
```{r}
bothtimes <- multisig_time(DE_res, ftissue = "lrt12")
```
...
#### AKH vs Virgin -- 6 hpm {.tabset .tabset-fade .tabset-pills} 
...
#### LVP vs AKH -- 6 hpm {.tabset .tabset-fade .tabset-pills}
...
#### AKH vs Virgin -- 24 hpm {.tabset .tabset-fade .tabset-pills}
...
#### LVP vs AKH -- 24 hpm {.tabset .tabset-fade .tabset-pills}
...
p <- sugar_df %>%
  ...
  geom_point() +
  scale_color_brewer(palette = "Dark2") +
  labs(x = "Log-fold change - at 3%", y = "Log-fold change - at 12%") +
  theme_bw(base_size = 14)
